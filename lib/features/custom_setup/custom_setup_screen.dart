// lib/features/custom_setup/custom_setup_screen.dart
...
class _CustomSetupScreenState extends State<CustomSetupScreen>
  ...
  // Tab index driven by quick-action 'scroll_xxx' actions
  static const _tabPairs       = 0;
  static const _tabIndicators  = 1;
  static const _tabDisplay     = 2;
  static const _tabRisk        = 3;
  ...
  Future<void> _save() async {
    ...
    if (mounted) {
      _showSnack(
          'Setup saved — ${_selectedPairs.length} pairs, '
          '${_selectedIndicators.length} indicators');
  ...
  void _togglePair(String pair) => setState(() {
  ...
  void _toggleIndicator(String ind) => setState(() {
  ...
  // Handle overlay quick-action — jump to the relevant tab
  void _handleQuickAction(dynamic action) {
    switch (action.routeOrAction as String) {
      case 'scroll_pairs':
        _tabController.animateTo(_tabPairs);
        break;
      case 'scroll_indicators':
        _tabController.animateTo(_tabIndicators);
        break;
      case 'scroll_display':
        _tabController.animateTo(_tabDisplay);
        break;
      case 'scroll_risk':
        _tabController.animateTo(_tabRisk);
        break;
      case 'save_and_launch':
        _save();
        break;
      default:
        if (action.isRoute as bool) {
          Navigator.pushNamed(context, action.routeOrAction as String);
        }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AppBackground(
      child: Scaffold(
        ...
        // ── body: overlay ON TOP of tab content ─────────────────────────
  ...
  PreferredSizeWidget _buildAppBar(bool isDark) {
    return AppBar(
      backgroundColor:
          isDark ? const Color(0xFF0A0C10) : Colors.white,
      elevation: 0,
      title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Custom Setup',
            style: TextStyle(
                fontSize: 18,
                fontWeight: FontWeight.w600,
                letterSpacing: -0.3)),
        Text(
          '${_selectedPairs.length} pairs · '
          '${_selectedIndicators.length} indicators',
          style: TextStyle(
              fontSize: 12,
              color: isDark
  ...
  // The overlay sits ABOVE the TabBarView in a Column so it is always visible
  // and responds regardless of which tab is active.
  Widget _buildBody(bool isDark) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        // ── Quick actions overlay (setup guide) ──────────────────────
      ...
      children: [
        ...
        Wrap(
          ...
          children: pairs
              .map((p) => _PairChip(
                    pair: p,
                    selected: selectedPairs.contains(p),
                    onTap: () => onToggle(p),
                    isDark: isDark,
                  ))
              .toList(),
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        ..._indicators.map((ind) => _IndicatorTile(
              code: ind.$1,
              description: ind.$2,
              selected: selectedIndicators.contains(ind.$1),
        ...
        Container(
          ...
          child: Row(children: [
            const Icon(Icons.info_outline,
                size: 16, color: Color(0xFF8A8880)),
            const SizedBox(width: 10),
            Expanded(
              child: Text(
                'Selected indicators appear on all chart views. '
                'Keep to 3–4 for clarity.',
                style: TextStyle(
                    fontSize: 12,
                    color: isDark
                        ? const Color(0xFF8A8880)
          ...
          child: Row(children: [
            ...
            if (selected)
              const Icon(Icons.check_circle_rounded,
                  color: Color(0xFFF0A500), size: 18)
            else
...
class _DisplayTab extends StatelessWidget {
  ...
  final String layoutMode, chartInterval;
  final void Function(String) onLayoutChanged, onIntervalChanged;
  final bool isDark;
    ...
    return ListView(
      ...
      children: [
        ...
        _InfoTile(
          ...
          subtitle:
              'TradingView integration coming in a future update',
          badge: 'Soon',
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Text(
          'This sets your default risk budget. '
          'You can fine-tune sliders in the AI Copilot.',
          style: TextStyle(
              fontSize: 13,
              color: isDark
                  ? const Color(0xFF8A8880)
        ...
        Container(
          ...
          child: const Row(
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  'Risk presets sync with the AI Risk Guardian. '
                  'Conservative mode enforces paper trading until '
                  'probation passes.',
                  style: TextStyle(
                      fontSize: 12,
                      color: Color(0xFF8A8880),
