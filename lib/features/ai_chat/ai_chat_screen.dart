...
class _AiChatScreenState extends State<AiChatScreen>
  ...
  void _addMessage(String text,
      {required bool isUser, String? intent, bool isCommand = false}) {
    setState(() {
  ...
  Future<void> _startVoice() async {
    if (_isListening || !_voiceSupported) {
      if (!_voiceSupported) {
        _addMessage(
            '🎙️ Voice not supported in this browser. Try Chrome or Edge.',
            isUser: false);
      }
      return;
    }
    ...
    try {
      ...
      _addMessage('🎙️ Could not hear clearly. Please try again.',
          isUser: false);
    } finally {
  ...
  Future<void> _handleVoiceCommand(String text) async {
    ...
    try {
      ...
      switch (intent) {
        case 'OPEN_TRADE':
          if (requiresConfirm) {
            _addMessage(response ?? 'Open trade?',
        ...
        case 'GET_SIGNAL':
          _addMessage(response ?? 'Fetching signals...',
              isUser: false, intent: intent);
          await _fetchAndDisplaySignal(parsed['pair'] as String?);
          break;

        case 'GET_NEWS':
          _addMessage(response ?? 'Fetching news...',
              isUser: false, intent: intent);
          await _sendMessage(
              'Give me the latest forex market news and what it means for trading today.');
          break;

        case 'GET_RISK':
          _addMessage(response ?? 'Calculating...',
              isUser: false, intent: intent);
          await _sendMessage(
              'Calculate Kelly Criterion position sizing for my current strategy (55% win rate, 1:1.67 RR).');
          break;

        case 'GET_PERFORMANCE':
          _addMessage(response ?? 'Fetching stats...',
              isUser: false, intent: intent);
  ...
  Future<void> _fetchAndDisplaySignal(String? pair) async {
    try {
      ...
      if (signals.isNotEmpty) {
        ...
        final conf =
            ((s['confidence'] as num? ?? 0.5) * 100).toStringAsFixed(0);
        ...
        actions: [
          TextButton(
            onPressed: () {
              ...
              _addMessage('Trade cancelled.', isUser: false);
            },
            child:
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              Navigator.pop(context);
              _addMessage('Opening $direction $pair paper trade...',
                  isUser: false);
              // Fetch live price and open trade
              try {
                final api = context.read<ApiService>();
                final prices = await api
                    .fetchLiveSignals(pairs: [parsed['pair'] ?? 'EUR_USD']);
                if (prices.isNotEmpty) {
                  final price =
                      (prices.first['entry_price'] as num?)?.toDouble() ?? 1.0;
                  ...
                  final signal = TradeSignal.fromBackend({
                    ...
                    'confidence': 0.7,
                    'sentiment': 'neutral',
                    'reasoning': 'Voice command trade',
                    'news_summary': '',
                    'generated_at': DateTime.now().toIso8601String(),
                  });
                  final success = await context
                      .read<PaperTradingProvider>()
  ...
  Widget _buildHeader() => Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border(
              bottom: BorderSide(color: Colors.white.withOpacity(0.157))),
        ),
        child: Row(children: [
          if (!widget.embeddedInPreview)
            IconButton(
          ...
          const Expanded(
            child:
              ...
              Text('Forex AI Copilot',
                  style: TextStyle(
                      color: Colors.white,
                      fontSize: 16,
                      fontWeight: FontWeight.w700)),
              Text('Voice + NLP • Phase 7',
            ...
            child: AnimatedBuilder(
              ...
              builder: (_, __) => Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color:
                        _isListening ? AppColors.primaryGreen : Colors.white24,
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: message.isUser
              ? null
              : Border.all(color: Colors.white.withOpacity(0.137)),
        ),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(_formatTime(message.timestamp),
              style: TextStyle(
                  color: Colors.white.withOpacity(0.47), fontSize: 10)),
        ]),
      ),
    );
  }
            ...
            SizedBox(
                ...
                child: CircularProgressIndicator(
                    ...
                    valueColor: AlwaysStoppedAnimation(
                        Colors.white.withOpacity(0.627)))),
            const SizedBox(width: 10),
  ...
  Widget _buildInputArea() => Container(
        ...
        decoration: BoxDecoration(
          ...
          border:
              Border(top: BorderSide(color: Colors.white.withOpacity(0.137))),
        ),
        child: Row(children: [
          Expanded(
            child: Container(
